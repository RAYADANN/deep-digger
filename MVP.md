...
> **Статус:** Фазы 0–3 закрыты 🟢, Фаза 4 — 🟡 в работе (единая логика глубины уже сделана).
...
- [ ] Переход **Dirt → Stone** (50 м): уведомление в HUD при первом пересечении границы (инфраструктура `Notification` + RemoteEvent `Notify` уже готовы).
- [ ] Цвет фона / окружения по слою (по возможности через `Lighting`).
- [ ] Штраф **x0.5** урона по Stone при `pickaxeLevel < 5` — добавить в `MiningEngine:hitBlock` (по `block.depth` или `oreDef.layer`).
- [ ] Сохранение **рекорда** `maxDepthReached` в профиле (текущая `depth` не сохраняется специально).
...
- [ ] **Дельта-синк блоков** (создан / обновлён HP / удалён) вместо полного списка каждый удар.
- [ ] Сервер: удар только по существующему открытому блоку (валидация координат).
- [ ] Лимит размера батча кликов в `MineBlock` (например, ≤16 за один Invoke).
- [ ] Доводка скрытых комнат: баланс шанса по глубине, уведомление по rarity.
- [ ] Профилирование: 10+ минут копания без просадок FPS.
